...
class AdvancedListFilters(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        return AdvancedFilter.objects.filter_by_user(request.user).filter(
            Q(model=model_name) | Q(created_at__year=2000)).order_by('-created_at').values_list('id', 'title')[:20]

    def queryset(self, request, queryset):
        if self.value():
            if request.GET.get('_afilter', False) and len(request.GET.get('_afilter', False)) > 15:
                try:
                    s = QSerializer(base64=True)
                    query = s.loads(request.GET.get('_afilter', False))
                    logger.debug(query.__dict__)
                    return queryset.filter(query).distinct()
                except Exception as e:
                    print('Error AdvancedListFilters lookups: %s' % e)
                    messages.error(request, _('An unexpected error has occurred. Please contact your system administrator. ') + 'Error AdvancedListFilters lookups: %s' % e)
                    return queryset.filter(id=0).distinct()
...
class AdminAdvancedFiltersMixin(object):
    ...
    def save_advanced_filter(self, request, form):
        if form.is_valid():
            ...
            if '_goto' in request.POST or '_goto' in request.GET:
                url = "{path}{qparams}".format(
                    path=request.path, qparams="?_afilter={qset}".format(qset=afilter.b64_query))
            ...
            if '_save_goto' in request.POST or '_save_goto' in request.GET:
                url = "{path}{qparams}".format(
